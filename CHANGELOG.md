...
- **H9 — my-listings incoming-inquiries grid.** `loadAll()` switched from `Promise.all` to `Promise.allSettled` so a single failing sibling fetch (REGA-auth/listings) can no longer blank the inquiries grid — the root cause of the "Convert to Deal" button failing to render for an OPEN inquiry. The `marketplace.cross-org` seller-convert spec is re-enabled (un-`fixme`).
