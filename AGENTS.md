...
## 1. User Preferences
- **NEVER add Co-Authored-By or any Codex/Anthropic attribution to git commits.** Omar Alghamdi is the sole author.
...
- After a pause, interruption, or context compaction, re-read CLAUDE.md/AGENTS.md before continuing non-trivial implementation, then resume from the latest verified blocker rather than assuming stale verification is still valid.
- **When the user declares prior materials obsolete, exclude them completely.** Do not reuse their screenshots, wording, structure, claims, visual system, or implicit assumptions as “helpful starting points.” Treat them as non-authoritative historical artifacts unless the user explicitly reinstates them. Build from current verified product truth instead.
...
**If any step cannot be completed** (preview won't start, no MCP preview tools available in this environment, etc.) — STOP, surface the blocker, and ask the user whether to proceed. Do NOT rationalize past the block.
...
- **Manual post-`db push` DDL lives in `packages/db/sql/` (not in `migrations/`).** `prisma db push` cannot express partial indexes, backfills, or RLS, so these are tracked `.sql` files applied by hand in the Supabase SQL Editor after a push, on every long-lived environment. Current scripts: `2026-06-partial-indexes.sql`, `2026-06-sequence-backfill.sql`, `2026-06-enable-rls.sql`. All are idempotent and safe to re-run.
  - **RLS coverage contract (learned 2026-06-12).** Mimarek reaches Postgres only via the owning `postgres` role (Prisma + `@prisma/adapter-pg`), which bypasses RLS — so RLS is enabled WITHOUT policies purely to slam the PostgREST/anon-key door (Supabase advisor `rls_disabled_in_public`). `2026-06-enable-rls.sql` must list **every** table in `schema.prisma` + the implicit M2M join table(s) (`_CouponPlans`) + `_prisma_migrations`. **The file is GENERATED by `packages/db/scripts/generate-rls.ts` (`npm run rls:generate` / `--write`) — it derives the table list straight from `schema.prisma`, so do NOT hand-edit it.** **When you add a model, regenerate the script in the same change** (CI runs `rls:check` and fails on drift) — otherwise the new table ships RLS-disabled and silently re-triggers the advisor (this is exactly how `SubscriptionMrrSnapshot`, the 3 marketplace tables, and the 2 counters drifted out of coverage). Never use `FORCE ROW LEVEL SECURITY` — it would route the owner through nonexistent policies and break every query. The resulting `rls_enabled_no_policy` (INFO) advisory on every such table is **expected and correct** — RLS-on + no-policy IS the firewall. Do NOT "resolve" it by adding a permissive policy (`USING (true)`); that re-opens the PostgREST/anon surface and undoes the fix.
  - **RLS is a required step of every schema change — handle it when needed, don't defer it (reaffirmed 2026-06-12).** Whenever a change adds, renames, or removes a table, treat RLS as part of that change, not an afterthought: **(1)** regenerate `2026-06-enable-rls.sql` via `npm run rls:generate` (it derives the table list from `schema.prisma` — never hand-edit; CI `rls:check` enforces) in the **same commit** (see the coverage contract above); **(2) apply the manual step on every long-lived environment** — paste the table's `ALTER TABLE IF EXISTS public."<Table>" ENABLE ROW LEVEL SECURITY;` line into the **Supabase SQL Editor** (CI's ephemeral DB doesn't need it, so prod silently drifts until you do — track which envs are done); **(3) verify it took** with `SELECT relname, relrowsecurity FROM pg_class WHERE relname = '<Table>';` (expect `relrowsecurity = t`). **Mixed-case Prisma table names MUST be double-quoted in raw SQL** — `public."ConsentLog"`, never `public.ConsentLog`. Unquoted identifiers fold to lowercase → `42P01: relation "public.consentlog" does not exist`, and worse, `ALTER TABLE IF EXISTS` then **silently no-ops** (returns "Success", RLS never enabled). If the Supabase advisor still shows `rls_disabled_in_public` after you "applied" it, you almost certainly hit the unquoted-no-op trap on that env. (Incident 2026-06-12: `ConsentLog` RLS appeared applied but the unquoted statement no-op'd; the quoted form fixed it.)
...
- **Customer PII must be written through the canonical encrypt path (v4.18.0).** Never call `customer.create`/`upsert` directly — route fresh customers through `app/actions/customers.ts` (`encryptCustomerData`) so phone/email/nationalId land AES-256-GCM-encrypted with their HMAC blind-index hashes. The marketplace inquiry helper (`marketplace.ts`) is the one other allowed creator. A `no-restricted-syntax` ESLint guard enforces this (incident: marketplace wrote plaintext `phone` + `"—"` placeholder, invisible to hash search — P1-1). Phone hashing uses `phoneSearchHash` (= `hashForSearch(normalizeSaudiPhoneE164(x) ?? x)`) on BOTH write and search sides — the single `lib/phone.ts` normalizer must be used everywhere a phone is hashed or rendered as a `tel:`/`wa.me` link.
...
- **Domain status/category/priority labels live in `apps/web/lib/domain-labels.ts` (v4.18.0 — F5), not per-page.** Canonical bilingual `{ ar, en }` label + badge-variant maps for `ContractStatus`, `ReservationStatus`, `PaymentStatus`, `MaintenanceCategory`/`Status`/`Priority`, and marketplace listing status — typed against the Prisma enums (a schema enum change breaks the build, not prod). **New pages MUST import from this registry; never re-declare a local copy.** This is the single place to apply a corrected Arabic term.
...
**Single source of truth (v4.18.0 — F4):** route → `{ permission, audience }` lives in **`apps/web/lib/route-guards.ts`** (`ROUTE_GUARDS`, pure edge-safe data — the only import allowed is the `Permission` *type*). `nav-items.ts`, the `auth.config.ts` edge gate (`audienceForPath`, longest-prefix), and `getTenantPageAccess()` all read from it. **When you add a route, add its `ROUTE_GUARDS` entry in the same change** — that is now the one place permission+audience are declared; the three consumers derive from it. The 403 middleware is still deferred (this only builds its seam).

1. **Navigation filter** — `navItems` in `apps/web/components/shell/nav-items.ts` derives each item's `permission`/`audience` from `ROUTE_GUARDS` (audience `"tenant" | "platform" | "shared"`). Sidebar / More / Cmd-K all filter on `isSystemRole(userRole)` before rendering. **Quick-action shortcuts must filter by audience too, not just permission.**
