...
- **Phase 5 — Release gate (§3.9):** 🟡 *partial.* Prod build green; key Phase 3–4 surfaces verified live (theme toggle works light+dark, notification filter pills, finance `Wk` axis, payments side-stripe-free + prod-stable, dark-mode clean) — **zero console errors**. ⬜ **Not yet done for the full gate / tag:** per-route 4-screenshot AR pass, mobile 375×812 pass on the Phase 3–4 changes, axe scan, keyboard tab-through; CHANGELOG `v4.11.0`; tag + GitHub release. **Tag deferred** (§3.9): v4.11 isn't feature-complete (ZATCA) and the full gate hasn't run — tagging now would repeat the v4.1.0 mistake.
