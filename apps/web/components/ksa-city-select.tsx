      ...
      {open && (
        <div className="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-md border bg-popover shadow-md">
          {filtered.length === 0 ? (
            ...
            filtered.map((city) => (
              // raw <button> intentional: these are listbox-option elements inside a custom
              // combobox dropdown. Button adds w-fit + whitespace-nowrap + fixed height (h-8/h-10)
              // which breaks the w-full dropdown layout and option row density. Migrating to Button
              // here would require overriding the variant entirely, defeating governance. Revisit if
              // a Combobox/ListboxOption primitive ships in @repo/ui.
              // eslint-disable-next-line react/forbid-elements -- listbox-option in custom combobox; see comment above
              <button
                key={city.value}
                type="button"
