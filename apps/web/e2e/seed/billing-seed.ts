...
async function main() {
  ...
  // Hidden plan (SEC-007 abuse fixture): isPublic=false. It must NEVER appear on
  // the customer plans page (getPublicPlans filters isPublic), and getPlanBySlug /
  // changePlan / subscribeToPlan must reject it. The e2e asserts it is absent.
  const hiddenPlan = await prisma.plan.upsert({
    where: { slug: "hidden-beta" },
    create: {
      slug: "hidden-beta",
      nameEn: "Hidden Beta Plan",
      nameAr: "خطة بيتا المخفية",
      descriptionEn: "Internal/non-public plan — must not be customer-selectable",
      descriptionAr: "خطة داخلية غير عامة — يجب ألا تكون قابلة للاختيار",
      priceMonthly: 1,
      priceAnnual: 10,
      trialDays: 0,
      isPublic: false,
  ...
  console.log(`  ✅ Hidden Beta (isPublic=false): ${hiddenPlan.id}`);
