...
export default function PortalClient({ initialSummary }: { initialSummary: PortalSummary | null }) {
  ...
  async function submitMaintenance() {
    ...
    try {
      ...
      if (!result.success) {
        toast.error(result.error ?? (lang === "ar" ? "تعذر إنشاء الطلب" : "Could not create request"));
        return;
      }
      setTitle("");
      setDescription("");
      toast.success(lang === "ar" ? "تم إرسال طلب الصيانة" : "Maintenance request submitted");
      router.refresh();
    } finally {
      setSubmitting(false);
    }
      ...
      <header className="border-b border-border bg-card">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-4 md:px-6">
          ...
          <div className="flex items-center gap-2">
            <ThemeToggle />
            <Button variant="secondary" size="sm" onClick={() => setLang(lang === "ar" ? "en" : "ar")} style={{ display: "inline-flex" }}>
        ...
        {loading ? (
          ...
          <>
                ...
                <CardContent className="space-y-5 p-5">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <h2 className="text-2xl font-bold">{summary.customer.name}</h2>
                    </div>
            ...
            <section className="grid gap-4 lg:grid-cols-2">
              <Card>
                <CardContent className="space-y-4 p-5">
                  <h3 className="flex items-center gap-2 font-semibold"><FileText className="h-5 w-5 text-primary" />{lang === "ar" ? "المستندات" : "Documents"}</h3>
                  {summary.documents.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{lang === "ar" ? "لا توجد مستندات متاحة." : "No documents are available."}</p>
                  ) : (
                    <div className="space-y-2">
                      {summary.documents.map((document) => (
                        <a key={document.id} href={document.url} className="flex items-center justify-between rounded-md border border-border p-3 text-sm hover:bg-muted/50" target="_blank" rel="noreferrer">
              ...
              <Card>
                <CardContent className="space-y-4 p-5">
                  <h3 className="flex items-center gap-2 font-semibold"><Wrench className="h-5 w-5 text-primary" />{lang === "ar" ? "طلب صيانة" : "Maintenance request"}</h3>
            ...
            <Card>
              <CardContent className="space-y-4 p-5">
                <h3 className="font-semibold">{lang === "ar" ? "متابعة الصيانة" : "Maintenance tracking"}</h3>
