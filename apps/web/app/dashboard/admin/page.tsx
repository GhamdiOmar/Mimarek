...
import {
  Building2, Users, Home, FileText, Receipt, BadgeDollarSign,
  Ticket, ListChecks, Tag, SearchCheck, Settings,
  ChevronRight, ShieldAlert, TrendingUp, AlertTriangle,
  TrendingDown, Wallet, ShieldCheck, PieChart, Sparkles,
} from "lucide-react";
...
export default function SystemAdminPage() {
  ...
  const load = React.useCallback(async () => {
    ...
    setLoading(true);
    try {
      const payload: { from: Date; to: Date } = { from: range.from, to: range.to };
      const [
        netNew,
        waterfall,
        aging,
        ...
        getNetNewArr(payload),
        getArrWaterfall(payload),
        getArAging(payload),
        getCollectedVsBilled(payload),
        getFailedPaymentArrAtRisk(payload),
        getZatcaClearanceRate(payload),
        getTopArrConcentration(payload),
        getDiscountLeakage(payload),
        getTrialToPaidConversion(payload),
  ...
  }, [authorized, range.from, range.to]);
  ...
  const arrSign = data?.netNew.valueSar ?? 0;
  ...
  const agingChartData = React.useMemo(() => {
    if (!data) return [];
    return data.aging.buckets.map((b) => ({
      bucket: b.label,
      value: b.sumSarGross,
    }));
  }, [data]);
  ...
  const mrrChartData = React.useMemo(() => {
    if (!data) return [];
    const now = new Date();
    return data.mrrTrend.map((total, i) => {
        ...
        {!authorized ? (
          <div className="flex-1 px-4 pt-10">
            <EmptyState
              ...
              description={
                lang === "ar"
                  ? "هذه الصفحة متاحة لفريق منصة ميماريك فقط."
      ...
      <div className="hidden md:block">
        ...
        >
          ...
          <KPICard
            ...
            value={loading ? "—" : fmtSarCompact(data?.netNew.valueSar)}
          ...
          <section>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <KPICard
                ...
                value={loading ? "—" : fmtSarCompact(data?.waterfall.startingArr)}
                unit={lang === "ar" ? "ر.س" : "SAR"}
                accent="info"
                locale={lang}
                loading={loading}
              ...
              <KPICard
                ...
                value={loading ? "—" : fmtSarCompact(data?.waterfall.newArr)}
                unit={lang === "ar" ? "ر.س" : "SAR"}
                accent="success"
                locale={lang}
                loading={loading}
              ...
              <KPICard
                ...
                value={loading ? "—" : fmtSarCompact(data?.waterfall.expansionArr)}
                unit={lang === "ar" ? "ر.س" : "SAR"}
                accent="secondary"
                locale={lang}
                loading={loading}
              ...
              <KPICard
                ...
                value={loading ? "—" : fmtSarCompact(data?.waterfall.reactivationArr)}
                unit={lang === "ar" ? "ر.س" : "SAR"}
                accent="primary"
                locale={lang}
                loading={loading}
              ...
              <KPICard
                ...
                value={loading ? "—" : fmtSarCompact(data?.waterfall.contractionArr)}
                unit={lang === "ar" ? "ر.س" : "SAR"}
                accent="warning"
                locale={lang}
                loading={loading}
              ...
              <KPICard
                ...
                value={loading ? "—" : fmtSarCompact(data?.waterfall.churnArr)}
                unit={lang === "ar" ? "ر.س" : "SAR"}
                accent="destructive"
                locale={lang}
                loading={loading}
              ...
              <KPICard
                ...
                value={loading ? "—" : fmtSarCompact(data?.waterfall.refundAdjArr)}
                unit={lang === "ar" ? "ر.س" : "SAR"}
                accent="destructive"
                locale={lang}
                loading={loading}
              ...
              <KPICard
                ...
                value={loading ? "—" : fmtSarCompact(data?.waterfall.endingArr)}
                unit={lang === "ar" ? "ر.س" : "SAR"}
                accent="info"
                locale={lang}
                loading={loading}
            ...
            {data && data.waterfall.reconciliationDrift > 1 && (
              <div className="mt-3 rounded-md border border-warning bg-warning/10 px-3 py-2 text-xs text-warning-foreground">
                ...
                {lang === "ar"
                  ? `تحذير المطابقة: انحراف ${fmtSarCompact(data.waterfall.reconciliationDrift)} ر.س — يجب أن تساوي الدلتا فرق نقاط البداية والنهاية.`
                  : `Reconciliation drift: ${fmtSarCompact(data.waterfall.reconciliationDrift)} SAR — deltas must equal Ending − Starting.`}
              </div>
            )}
          ...
          <section>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <KPICard
                ...
                value={loading ? "—" : fmtSarCompact(data?.collected.collectedSarGross)}
                unit={lang === "ar" ? "ر.س" : "SAR"}
                accent="success"
                comparisonPeriod={
                  data
                    ? lang === "ar"
                      ? `من ${fmtSarCompact(data.collected.billedSarGross)} مُصدر`
                      : `of ${fmtSarCompact(data.collected.billedSarGross)} billed`
                    : undefined
                }
                locale={lang}
                loading={loading}
              ...
              <KPICard
                ...
                value={
                  loading
                    ? "—"
                    : fmtSarCompact(data?.aging.buckets[0]?.sumSarGross ?? 0)
                }
                unit={lang === "ar" ? "ر.س" : "SAR"}
                accent="info"
                locale={lang}
              ...
              <KPICard
                ...
                value={
                  loading
                    ? "—"
                    : fmtSarCompact(data?.aging.buckets[1]?.sumSarGross ?? 0)
                }
                unit={lang === "ar" ? "ر.س" : "SAR"}
                accent="warning"
                locale={lang}
              ...
              <KPICard
                ...
                value={
                  loading
                    ? "—"
                    : fmtSarCompact(data?.aging.buckets[2]?.sumSarGross ?? 0)
                }
                unit={lang === "ar" ? "ر.س" : "SAR"}
                accent="warning"
                locale={lang}
              ...
              <KPICard
                ...
                value={
                  loading
                    ? "—"
                    : fmtSarCompact(data?.aging.buckets[3]?.sumSarGross ?? 0)
                }
                unit={lang === "ar" ? "ر.س" : "SAR"}
                accent="destructive"
                locale={lang}
              ...
              <KPICard
                ...
                value={loading ? "—" : fmtSarCompact(data?.atRisk.atRiskArrSar)}
                unit={lang === "ar" ? "ر.س" : "SAR"}
                accent="destructive"
                comparisonPeriod={
                  data
                    ? lang === "ar"
                      ? `${fmtCount(data.atRisk.count)} اشتراك`
                      : `${fmtCount(data.atRisk.count)} subs`
                    : undefined
                }
                locale={lang}
                loading={loading}
              ...
              <KPICard
                ...
                value={loading ? "—" : fmtPct(data?.zatca.rate)}
                accent={
                  data?.zatca.alertSpike
                    ? "destructive"
                    : data?.zatca.rate != null && data.zatca.rate >= 0.99
                      ? "success"
                      : "warning"
                }
                comparisonPeriod={
          ...
          <section>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <KPICard
                ...
                value={loading ? "—" : fmtCount(data?.risk.pastDueCount)}
                accent="destructive"
              ...
              <KPICard
                ...
                value={loading ? "—" : fmtCount(data?.risk.noLoginAdminCount)}
                accent="warning"
              ...
              <KPICard
                ...
                value={loading ? "—" : fmtCount(data?.risk.openP1OldCount)}
                accent="warning"
              ...
              <KPICard
                ...
                value={loading ? "—" : fmtCount(data?.risk.failedPaymentCount)}
                accent="destructive"
          ...
          <section>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <KPICard
                ...
                value={loading ? "—" : fmtPct(data?.concentration.top5Pct)}
                accent="warning"
              ...
              <KPICard
                ...
                value={loading ? "—" : fmtPct(data?.concentration.top10Pct)}
                accent="info"
                icon={<PieChart className="h-[18px] w-[18px]" />}
                locale={lang}
                loading={loading}
              ...
              <KPICard
                ...
                value={loading ? "—" : fmtPct(data?.leakage.leakagePct)}
                accent="warning"
              ...
              <KPICard
                ...
                value={loading ? "—" : fmtPct(data?.trial.rate)}
                accent="primary"
                icon={<Sparkles className="h-[18px] w-[18px]" />}
                comparisonPeriod={
                  data
                    ? lang === "ar"
                      ? `من ${fmtCount(data.trial.denominator)} تجربة`
                      : `of ${fmtCount(data.trial.denominator)} trials`
                    : undefined
                }
                locale={lang}
                loading={loading}
          ...
          <section>
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              <KPICard
                ...
                value={loading ? "—" : fmtCount(data?.stats.orgCount)}
                accent="primary"
                locale={lang}
                loading={loading}
              />
              <KPICard
                ...
                value={loading ? "—" : fmtCount(data?.stats.userCount)}
                accent="info"
                locale={lang}
                loading={loading}
              />
              <KPICard
                ...
                value={loading ? "—" : fmtCount(data?.stats.propertyCount)}
                accent="secondary"
                locale={lang}
                loading={loading}
              />
              <KPICard
                ...
                value={loading ? "—" : fmtCount(data?.stats.contractCount)}
                accent="primary"
                locale={lang}
                loading={loading}
              />
              <KPICard
                ...
                value={
                  loading
                    ...
                    : fmtCount(
                        data
                          ? data.stats.openTickets + data.stats.inProgressTickets
                          : null,
                      )
                }
                accent="warning"
