...
type MarketplaceViewProps = {
  initialListings: MarketplaceListingCardVM[];
  initialSellerOrgs: { id: string; name: string | null }[];
};

export default function MarketplaceView(props: MarketplaceViewProps) {
  return (
    <React.Suspense
      fallback={
...
function MarketplacePage({ initialListings, initialSellerOrgs }: MarketplaceViewProps) {
  ...
  // Data state
  const [listings, setListings] = React.useState<MarketplaceListingCardVM[]>(initialListings);
  const [sellerOrgs, setSellerOrgs] = React.useState<{ id: string; name: string | null }[]>(initialSellerOrgs);
  ...
  // Reload listings whenever URL search params change. Skip the FIRST run —
  // the initial listings came from the RSC server shell with the current
  // searchParams (CX-003 pt1); re-fetch only when the user changes a filter.
