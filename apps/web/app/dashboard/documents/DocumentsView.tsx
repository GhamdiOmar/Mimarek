...
export default function DocumentsView({ initialDocs }: DocumentsViewProps) {
  ...
  // Initial documents arrive as props from the RSC server shell (CX-003 pt1 —
  // no first-paint client mount-fetch). The list refreshes after an upload via
  // getDocuments() in handleUploadComplete below.
  ...
  // Post-mutation refresh (initial docs come from props via the RSC shell).
  // Drives `loading`/`loadError` so the skeleton + error states stay functional.
